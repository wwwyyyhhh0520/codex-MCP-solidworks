...
bool pdfDelivery=argMode=="--pdf-delivery";
...
bool supportedMode=probe||pdfDelivery||argMode=="--execute"||manual||ownedExecution||saveDiagnostic||saveDiagnosticFullOnly;
if(!supportedMode)
{
...
int expectedArgumentCount=pdfDelivery?7:(manualSession?7:(ownedExecution||saveDiagnostic||saveDiagnosticFullOnly?6:0));
...
string semanticsPath=pdfDelivery?args[2]:(probe?args[1]:args[2]),template=pdfDelivery?args[4]:(probe?args[2]:args[3]),output=Path.GetFullPath(pdfDelivery?args[5]:(probe?args[3]:args[4]));
string planPath=pdfDelivery?args[1]:args[1];
string explicitModelPath=pdfDelivery?args[3]:string.Empty;
...
try {
    ...
    SemanticExecutionFacts? sharedFacts = null;
    if (pdfDelivery)
    {
        Console.WriteLine("SEMANTIC_FACTS_LOAD_START");
        using var sharedPlanDocument = JsonDocument.Parse(File.ReadAllText(planPath));
        sharedFacts = TryLoadSemanticExecutionFacts(sharedPlanDocument.RootElement, sem, semanticsPath);
        Console.WriteLine($"SEMANTIC_FACTS_LOAD_RESULT plan_definition_count={sharedFacts.PlanDefinitionCount};hole_semantic_count={sharedFacts.HoleSemantics.Count};physical_opening_count={sharedFacts.PhysicalOpenings.Count};frame_evidence_available={(sharedFacts.FrameEvidence is not null).ToString().ToUpperInvariant()};datum_evidence_available={(sharedFacts.DatumEvidence is not null).ToString().ToUpperInvariant()};status=PASS");
        Console.WriteLine("SEMANTIC_FACTS_LOAD_COUNT=1");
    }
    if(pdfDelivery){
        if(!File.Exists(planPath)) throw new InvalidOperationException("PLAN_NOT_FOUND");
        using var planJson=JsonDocument.Parse(File.ReadAllText(planPath));
        var modelFull=Path.GetFullPath(explicitModelPath);
        if(!File.Exists(modelFull)) throw new InvalidOperationException("MODEL_NOT_FOUND");
        if(!File.Exists(template)) throw new InvalidOperationException("TEMPLATE_NOT_FOUND");
        var semanticModel=sem.TryGetProperty("model_path",out var mp)?Path.GetFullPath(mp.GetString()??""):"";
        if(!string.Equals(semanticModel,modelFull,StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("SEMANTICS_IDENTITY_MISMATCH");
        Console.WriteLine($"PLAN_JSON_PARSE=PASS;SEMANTICS_JSON_PARSE=PASS;SEMANTICS_PATH={Path.GetFullPath(semanticsPath)};SEMANTICS_IDENTITY_MATCH=YES;MODEL_PATH={modelFull};MODEL_SHA256={Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(File.ReadAllBytes(modelFull)))};TEMPLATE_SHA256={Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(File.ReadAllBytes(template)))}");
    }
    ...
    int err=0,warn=0;
    var modelPath=pdfDelivery?Path.GetFullPath(explicitModelPath):sem.GetProperty("model_path").GetString()!;
    ...
    ModelDoc2 dm;
    bool existingBase=!pdfDelivery&&!probe&&!manual&&args.Length==7;
    ...
    var math=(MathUtility)sw.GetMathUtility();var holes=pdfDelivery && sharedFacts is not null ? sharedFacts.HoleSemantics : LoadHoleSemantics(semanticsPath);
    ...
    if (pdfDelivery)
    {
        Console.WriteLine($"ACTIVE_DOC_TYPE={dm.GetType()}");
        Console.WriteLine($"ACTIVE_SHEET_NAME={sheet.GetName()}");
        var templateNameValue = sheet.GetType().GetMethod("GetTemplateName")?.Invoke(sheet, null)?.ToString() ?? "<unavailable>";
        Console.WriteLine($"SHEET_TEMPLATE_NAME={templateNameValue}");
        var rawProperties = sheet.GetType().GetMethod("GetProperties")?.Invoke(sheet, null);
        Console.WriteLine($"SHEET_PROPERTIES_RAW={rawProperties ?? "<unavailable>"}");
        Console.WriteLine($"SHEET_WIDTH_M={sheetW:R}");
        Console.WriteLine($"SHEET_HEIGHT_M={sheetH:R}");
        var sheetScaleNumerator = 1.0; var sheetScaleDenominator = 1.0;
        if (rawProperties is Array properties && properties.Length >= 2)
        {
            sheetScaleNumerator = Convert.ToDouble(properties.GetValue(0));
            sheetScaleDenominator = Convert.ToDouble(properties.GetValue(1));
        }
        Console.WriteLine($"SHEET_SCALE_NUMERATOR={sheetScaleNumerator:R}");
        Console.WriteLine($"SHEET_SCALE_DENOMINATOR={sheetScaleDenominator:R}");
        Console.WriteLine($"SHEET_FORMAT_REFERENCE_EXISTS={(!string.IsNullOrWhiteSpace(templateNameValue) && File.Exists(templateNameValue)).ToString().ToUpperInvariant()}");

        var beforeBounds = new Dictionary<string, double[]>(StringComparer.OrdinalIgnoreCase);
        foreach (var pair in owners)
        {
            var outline = ToDoubles(pair.Value.GetOutline());
            beforeBounds[pair.Key] = outline;
            Console.WriteLine($"VIEW_ROLE={pair.Key}");
            Console.WriteLine($"VIEW_NAME={pair.Value.Name}");
            Console.WriteLine($"VIEW_SCALE_DECIMAL_BEFORE={pair.Value.ScaleDecimal:R}");
            Console.WriteLine($"VIEW_POSITION_BEFORE={JsonSerializer.Serialize(ToDoubles(pair.Value.Position))}");
            Console.WriteLine($"VIEW_OUTLINE_BEFORE={JsonSerializer.Serialize(outline)}");
            Console.WriteLine($"VIEW_WIDTH_BEFORE={outline[2]-outline[0]:R}");
            Console.WriteLine($"VIEW_HEIGHT_BEFORE={outline[3]-outline[1]:R}");
        }

        var fitMargin = Math.Max(Math.Min(sheetW, sheetH) * 0.05, 1e-4);
        var annotationClearance = Math.Max(Math.Min(sheetW, sheetH) * 0.08, 1e-4);
        var usableLeft = fitMargin; var usableBottom = fitMargin;
        var usableRight = Math.Max(usableLeft, sheetW - fitMargin); var usableTop = Math.Max(usableBottom, sheetH - fitMargin - annotationClearance);
        var beforeLayout = beforeBounds.Values.ToArray();
        var regionWidth = (usableRight - usableLeft) * 0.42;
        var regionHeight = (usableTop - usableBottom) * 0.38;
        var capacities = new List<double>();
        foreach (var pair in owners)
        {
            var b = beforeBounds[pair.Key]; var width = Math.Max(b[2]-b[0], 1e-9); var height = Math.Max(b[3]-b[1], 1e-9);
            var fitX = regionWidth / width; var fitY = regionHeight / height; var capacity = Math.Min(fitX, fitY);
            capacities.Add(capacity);
            Console.WriteLine($"VIEW_ROLE={pair.Key}"); Console.WriteLine($"CURRENT_VIEW_WIDTH_M={width:R}"); Console.WriteLine($"CURRENT_VIEW_HEIGHT_M={height:R}");
            Console.WriteLine($"ASSIGNED_REGION_WIDTH_M={regionWidth:R}"); Console.WriteLine($"ASSIGNED_REGION_HEIGHT_M={regionHeight:R}");
            Console.WriteLine($"FIT_MULTIPLIER_X={fitX:R}"); Console.WriteLine($"FIT_MULTIPLIER_Y={fitY:R}"); Console.WriteLine($"VIEW_MULTIPLIER_CAPACITY={capacity:R}");
        }
        var commonMultiplierBeforeSafety = capacities.Min();
        var fitMultiplier = commonMultiplierBeforeSafety * 0.90;
        Console.WriteLine($"COMMON_MULTIPLIER_BEFORE_SAFETY={commonMultiplierBeforeSafety:R}");
        Console.WriteLine($"COMMON_MULTIPLIER_AFTER_SAFETY={fitMultiplier:R}");
        fitMultiplier = Math.Max(fitMultiplier, 1.0);
        var requestedScales = new Dictionary<string,double>(StringComparer.OrdinalIgnoreCase);
        foreach (var pair in owners)
        {
            var initial = beforeBounds[pair.Key];
            var initialWidth = initial[2] - initial[0]; var initialHeight = initial[3] - initial[1];
            var initialScale = pair.Value.ScaleDecimal;
            var targetScale = Math.Max(initialScale * fitMultiplier, 1e-6);
            requestedScales[pair.Key] = targetScale;
            var useSheet = pair.Value.UseSheetScale;
            var parentProperty = pair.Value.GetType().GetProperty("UseParentScale");
            var useParent = parentProperty?.GetValue(pair.Value)?.ToString() ?? "<unavailable>";
            Console.WriteLine($"VIEW_ROLE={pair.Key}");
            Console.WriteLine($"VIEW_NAME={pair.Value.Name}");
            Console.WriteLine($"VIEW_SCALE_INITIAL={initialScale:R}");
            Console.WriteLine($"VIEW_OUTLINE_INITIAL={JsonSerializer.Serialize(initial)}");
            Console.WriteLine($"VIEW_WIDTH_INITIAL={initialWidth:R}");
            Console.WriteLine($"VIEW_HEIGHT_INITIAL={initialHeight:R}");
            Console.WriteLine($"VIEW_USES_SHEET_SCALE={useSheet}");
            Console.WriteLine($"VIEW_USE_PARENT_SCALE={useParent}");
            Console.WriteLine($"CALCULATED_COMMON_SCALE_MULTIPLIER={fitMultiplier:R}");
            Console.WriteLine($"CALCULATED_TARGET_VIEW_SCALE={targetScale:R}");
            pair.Value.UseSheetScale = 0;
            pair.Value.ScaleDecimal = targetScale;
            Console.WriteLine($"VIEW_SCALE_ASSIGNED={pair.Value.ScaleDecimal:R}");
        }
        dm.EditRebuild3();
        foreach (var pair in owners)
        {
            var initial = beforeBounds[pair.Key];
            var initialWidth = initial[2] - initial[0]; var initialHeight = initial[3] - initial[1];
            var after = ToDoubles(pair.Value.GetOutline());
            var afterWidth = after[2] - after[0]; var afterHeight = after[3] - after[1];
            var expectedRatio = pair.Value.ScaleDecimal / Math.Max(1e-12, pair.Value.ScaleDecimal / fitMultiplier);
            var actualWidthRatio = afterWidth / Math.Max(1e-12, initialWidth);
            var actualHeightRatio = afterHeight / Math.Max(1e-12, initialHeight);
            var widthRelativeError = Math.Abs(actualWidthRatio-expectedRatio)/Math.Max(Math.Abs(expectedRatio),1e-9);
            var heightRelativeError = Math.Abs(actualHeightRatio-expectedRatio)/Math.Max(Math.Abs(expectedRatio),1e-9);
            Console.WriteLine($"VIEW_SCALE_AFTER_REBUILD={pair.Value.ScaleDecimal:R}");
            Console.WriteLine($"VIEW_OUTLINE_AFTER_REBUILD={JsonSerializer.Serialize(after)}");
            Console.WriteLine($"VIEW_WIDTH_AFTER_REBUILD={afterWidth:R}");
            Console.WriteLine($"VIEW_HEIGHT_AFTER_REBUILD={afterHeight:R}");
            Console.WriteLine($"EXPECTED_WIDTH_RATIO={expectedRatio:R}");
            Console.WriteLine($"ACTUAL_WIDTH_RATIO={actualWidthRatio:R}");
            Console.WriteLine($"EXPECTED_HEIGHT_RATIO={expectedRatio:R}");
            Console.WriteLine($"ACTUAL_HEIGHT_RATIO={actualHeightRatio:R}");
            Console.WriteLine($"WIDTH_RATIO_RELATIVE_ERROR={widthRelativeError:R}");
            Console.WriteLine($"HEIGHT_RATIO_RELATIVE_ERROR={heightRelativeError:R}");
            Console.WriteLine("SCALE_RATIO_DIAGNOSTIC_ONLY=YES");
        }
        Console.WriteLine($"SHEET_WIDTH_M={sheetW:R}");
        Console.WriteLine($"SHEET_HEIGHT_M={sheetH:R}");
        Console.WriteLine($"USABLE_SHEET_WIDTH_M={usableRight-usableLeft:R}");
        Console.WriteLine($"USABLE_SHEET_HEIGHT_M={usableTop-usableBottom:R}");
        Console.WriteLine("VIEW_SCALE_EFFECT_MATCH=DEFERRED_UNTIL_FINAL_GEOMETRY_VALIDATION");
        var orderedRoles = owners.Keys.OrderBy(x => x, StringComparer.OrdinalIgnoreCase).ToArray();
        var anchorRole = orderedRoles.FirstOrDefault(r => r.Equals("front", StringComparison.OrdinalIgnoreCase)) ?? orderedRoles.First();
        var anchorBox = ToDoubles(owners[anchorRole].GetOutline());
        var anchorW = anchorBox[2] - anchorBox[0]; var anchorH = anchorBox[3] - anchorBox[1];
        var anchorX = (usableLeft + usableRight) * 0.5; var anchorY = (usableBottom + usableTop) * 0.55;
        owners[anchorRole].Position = new[]{anchorX, anchorY};
        foreach (var role in orderedRoles.Where(r => !r.Equals(anchorRole, StringComparison.OrdinalIgnoreCase)))
        {
            var view = owners[role]; var box = ToDoubles(view.GetOutline());
            var w = box[2]-box[0]; var h = box[3]-box[1];
            var lower = role.Equals("top", StringComparison.OrdinalIgnoreCase) || role.Equals("bottom", StringComparison.OrdinalIgnoreCase);
            var leftRight = role.Equals("left", StringComparison.OrdinalIgnoreCase) || role.Equals("right", StringComparison.OrdinalIgnoreCase);
            var cx = lower ? anchorX : role.Equals("left", StringComparison.OrdinalIgnoreCase) ? anchorX - (anchorW+w)*0.5 - fitMargin : anchorX + (anchorW+w)*0.5 + fitMargin;
            var cy = leftRight ? anchorY : role.Equals("top", StringComparison.OrdinalIgnoreCase) ? anchorY + (anchorH+h)*0.5 + fitMargin : anchorY - (anchorH+h)*0.5 - fitMargin;
            if (!lower && !leftRight) { cx = anchorX; cy = anchorY - (anchorH+h)*0.5 - fitMargin; }
            view.Position = new[]{Math.Clamp(cx, usableLeft+w/2, usableRight-w/2), Math.Clamp(cy, usableBottom+h/2, usableTop-h/2)};
            Console.WriteLine($"ORTHO_ALIGNMENT_PAIR={anchorRole}->{role}");
            Console.WriteLine($"ORTHO_ALIGNMENT_AXIS={(lower ? "X_CENTER" : "Y_CENTER")}");
            Console.WriteLine($"ORTHO_ALIGNMENT_ERROR_M={(lower ? Math.Abs(ToDoubles(view.Position)[0]-ToDoubles(owners[anchorRole].Position)[0]) : Math.Abs(ToDoubles(view.Position)[1]-ToDoubles(owners[anchorRole].Position)[1])):R}");
        }
        dm.EditRebuild3(); dm.ForceRebuild3(false);
        var afterBounds = new Dictionary<string,double[]>(StringComparer.OrdinalIgnoreCase);
        foreach (var pair in owners)
        {
            var outline = ToDoubles(pair.Value.GetOutline()); afterBounds[pair.Key]=outline;
            Console.WriteLine($"VIEW_ROLE={pair.Key}"); Console.WriteLine($"VIEW_SCALE_DECIMAL_AFTER={pair.Value.ScaleDecimal:R}");
            Console.WriteLine($"VIEW_POSITION_AFTER={JsonSerializer.Serialize(ToDoubles(pair.Value.Position))}"); Console.WriteLine($"VIEW_OUTLINE_AFTER={JsonSerializer.Serialize(outline)}");
        }
        bool inside(double[] b)=>b[0]>=usableLeft&&b[1]>=usableBottom&&b[2]<=usableRight&&b[3]<=usableTop;
        bool overlap(double[] a,double[] b)=>Math.Min(a[2],b[2])>Math.Max(a[0],b[0])+1e-9&&Math.Min(a[3],b[3])>Math.Max(a[1],b[1])+1e-9;
        var afterValues=afterBounds.Values.ToArray(); var overlapCount=Enumerable.Range(0,afterValues.Length).Sum(i=>afterValues.Skip(i+1).Count(b=>overlap(afterValues[i],b)));
        var allInside=afterValues.All(inside); var allLeft=afterValues.Min(b=>b[0]); var allBottom=afterValues.Min(b=>b[1]); var allRight=afterValues.Max(b=>b[2]); var allTop=afterValues.Max(b=>b[3]);
        Console.WriteLine($"SHEET_USABLE_BOUNDS={JsonSerializer.Serialize(new[]{usableLeft,usableBottom,usableRight,usableTop})}"); Console.WriteLine($"VIEW_LAYOUT_BOUNDS={JsonSerializer.Serialize(new[]{allLeft,allBottom,allRight,allTop})}");
        Console.WriteLine($"ALL_VIEWS_INSIDE_USABLE_SHEET={allInside.ToString().ToUpperInvariant()}"); Console.WriteLine($"VIEW_OVERLAP_COUNT={overlapCount}");
        Console.WriteLine($"VIEW_LAYOUT_OCCUPANCY_X={(allRight-allLeft)/(usableRight-usableLeft):R}"); Console.WriteLine($"VIEW_LAYOUT_OCCUPANCY_Y={(allTop-allBottom)/(usableTop-usableBottom):R}");
        Console.WriteLine($"VIEW_GEOMETRY_OCCUPANCY_X={(allRight-allLeft)/(usableRight-usableLeft):R}");
        Console.WriteLine($"VIEW_GEOMETRY_OCCUPANCY_Y={(allTop-allBottom)/(usableTop-usableBottom):R}");
        Console.WriteLine($"VIEW_GEOMETRY_OCCUPANCY_AREA={((allRight-allLeft)*(allTop-allBottom))/((usableRight-usableLeft)*(usableTop-usableBottom)):R}");
        Console.WriteLine("ANNOTATION_BANDS_IMPLEMENTED=NO");
        Console.WriteLine("CENTRAL_CORRIDOR_RESERVATION_IMPLEMENTED=NO");
        var scalePropertyMatch = owners.All(p => double.IsFinite(p.Value.ScaleDecimal) && requestedScales.TryGetValue(p.Key, out var requested) && Math.Abs(p.Value.ScaleDecimal-requested) <= Math.Max(1e-9, Math.Abs(requested)*1e-6));
        var outlineValid = afterValues.All(b => b.Length >= 4 && b.All(double.IsFinite) && b[2] > b[0] && b[3] > b[1]);
        var regionFit = allInside;
        var geometryOverlap = overlapCount != 0;
        Console.WriteLine($"VIEW_SCALE_PROPERTY_MATCH={(scalePropertyMatch ? "YES" : "NO")}");
        Console.WriteLine($"VIEW_OUTLINE_VALID={(outlineValid ? "YES" : "NO")}");
        Console.WriteLine($"VIEW_REGION_FIT={(regionFit ? "YES" : "NO")}");
        Console.WriteLine($"VIEW_GEOMETRY_OVERLAP={(geometryOverlap ? "YES" : "NO")}");
        var finalScaleAccepted = scalePropertyMatch && outlineValid && regionFit && !geometryOverlap;
        Console.WriteLine($"VIEW_SCALE_EFFECT_MATCH={(finalScaleAccepted ? "YES" : "NO")}");
        if (!finalScaleAccepted) throw new InvalidOperationException("VIEW_SCALE_APPLICATION_BLOCKER=FINAL_GEOMETRY_VALIDATION_FAILED");
        Console.WriteLine("DIMENSION_PIPELINE_ENTER=YES");
    ...
    DrawingExecutionContext? drawingExecutionContext = null;
    if (pdfDelivery && sharedFacts is not null)
    {
        drawingExecutionContext = new DrawingExecutionContext(sharedFacts, part, dm, drawing, owners, math);
        Console.WriteLine("DRAWING_EXECUTION_CONTEXT_CREATED=YES");
        Console.WriteLine($"DRAWING_EXECUTION_CONTEXT_VIEW_COUNT={drawingExecutionContext.RuntimeViews.Count}");
        Console.WriteLine($"CONTEXT_HAS_SEMANTICS={(drawingExecutionContext.Facts.Semantics.ValueKind != JsonValueKind.Undefined ? "YES" : "NO")}");
        Console.WriteLine($"CONTEXT_HAS_HOLE_CATALOG={(drawingExecutionContext.Facts.HoleSemantics.Count > 0 ? "YES" : "NO")}");
        Console.WriteLine($"CONTEXT_HAS_PHYSICAL_OPENINGS={(drawingExecutionContext.Facts.PhysicalOpenings.Count > 0 ? "YES" : "NO")}");
        Console.WriteLine($"CONTEXT_HAS_FRAME_EVIDENCE={(drawingExecutionContext.Facts.FrameEvidence is not null ? "YES" : "NO")}");
        Console.WriteLine($"CONTEXT_HAS_DATUM_EVIDENCE={(drawingExecutionContext.Facts.DatumEvidence is not null ? "YES" : "NO")}");
    }
    ...
    foreach(var intent in plan.GetProperty("intents").EnumerateArray().OrderBy(x=>x.GetProperty("kind").GetString()=="CALLOUT"?1:0)) {
        ...
        try {
            ...
            OverallProbeResult? acceptedOverallProbe = null;
            if(kind=="OVERALL") {
                ...
                OverallProbeResult? acceptedProbe = proven is not null
                    ? new OverallProbeResult(proven.A, proven.B, proven.Pa, proven.Pb, proven.Score, "PROVENANCE_REBIND", targetAxisSpan, targetAxisSpan)
                    : null;
                ...
                if (proven is null)
                {
                    ...
                    {
                        var outcome = ProbeOverallCandidate(candidate, dm, math, v, identity, role, key, ai, orientation, targetAxisSpan, expected.Value, raw.Count, overallCreateClock);
                        if (outcome.Kind == OverallProbeOutcomeKind.Accepted) { acceptedProbe = outcome.Result; break; }
                        if (outcome.Kind is OverallProbeOutcomeKind.NormalRejected or OverallProbeOutcomeKind.ExceptionContinue) continue;
                    }
                }
                if(acceptedProbe is null)
                {
                ...
                acceptedOverallProbe = acceptedProbe;
                var selectedA = acceptedProbe.A;
                var selectedB = acceptedProbe.B;
            ...
            var resolvedIntent = TryResolveAnnotationIntent(d, role, g, expected.HasValue, out var unresolvedReason);
            if (resolvedIntent is null)
            {
                Console.WriteLine($"RESOLVED_INTENT_RESULT annotation_id={identity};kind={kind};owner_view_role={role};geometry_resolution_status={(g is null ? "UNRESOLVED" : "RESOLVED")};semantic_target_status={(expected.HasValue ? "RESOLVED" : "UNRESOLVED")};resolution_status=UNRESOLVED;reason={unresolvedReason}");
                throw new InvalidOperationException(unresolvedReason);
            }
            object result;
            DisplayDimension? display;
            string createdId;
            double? committedSystemValue = null;
            if(kind=="OVERALL")
            {
                var commit = CommitOverallAnnotation(acceptedOverallProbe!, dm, math, v, identity, role, key, orientation, expected!.Value, g.CandidateCount, overallBindingStrategy);
                g = commit.Geometry;
                d = commit.Definition;
                result = commit.NativeResult;
                display = commit.Display;
                committedSystemValue = commit.NativeSystemValueM;
                createdId = commit.CreationIdentity;
            }
            else
            {
                if(kind=="CALLOUT") result=CreateHoleCallout(dm,v,d,g,0);
                else result=CreatePitchDimension(dm,v,d,g,0);
                var je=JsonSerializer.SerializeToElement(result);createdId=je.GetProperty("creation_identity").GetString()!;
                display=null;for(var dd=v.GetFirstDisplayDimension5() as DisplayDimension;dd!=null;dd=dd.GetNext5() as DisplayDimension)if(dd.GetNameForSelection()==createdId)display=dd;
            }
            if(display==null)throw new Exception("CREATED_DISPLAY_MISSING");
            if(expected.HasValue && committedSystemValue is null)
            {
    ...
    var positionAudit = new List<(PolicyAnnotation Annotation,double[] Before,double[] Assigned,string Band,double[] Slot)>();
    Console.WriteLine("ANNOTATION_LAYOUT_STAGE_ENTER=YES");
    Console.WriteLine($"ANNOTATION_LAYOUT_INPUT_COUNT={annotations.Count}");
    var movableAnnotationCount = annotations.Count(a => a.Display is not null && a.Display.GetAnnotation() is Annotation);
    Console.WriteLine($"ANNOTATION_LAYOUT_MOVABLE_COUNT={movableAnnotationCount}");
    if (annotations.Count == 0) Console.WriteLine("ANNOTATION_LAYOUT_BLOCKER=EMPTY_RUNTIME_ANNOTATION_COLLECTION");
    else if (movableAnnotationCount == 0) Console.WriteLine("ANNOTATION_LAYOUT_BLOCKER=NO_MOVABLE_NATIVE_ANNOTATIONS");
    foreach(var a in annotations.OrderBy(x=>x.Kind=="CALLOUT"?1:0).ThenBy(x=>x.Kind=="OVERALL"?1:0)) {
        ...
        var beforePosition=AnnotationPosition(a.Display);
        Console.WriteLine($"ANNOTATION_ID={a.Id}"); Console.WriteLine($"ANNOTATION_KIND={a.Kind}"); Console.WriteLine($"OWNER_VIEW_ROLE={a.Role}");
        Console.WriteLine($"TEXT_POSITION_BEFORE_X={(beforePosition.Length>0?beforePosition[0]:double.NaN):R}"); Console.WriteLine($"TEXT_POSITION_BEFORE_Y={(beforePosition.Length>1?beforePosition[1]:double.NaN):R}");
        ...
        Console.WriteLine($"ASSIGNED_BAND={best.Side}"); Console.WriteLine($"ASSIGNED_SLOT_X={best.Position[0]:R}"); Console.WriteLine($"ASSIGNED_SLOT_Y={best.Position[1]:R}");
        Console.WriteLine($"BAND_TOP_Y={usable[3]:R}"); Console.WriteLine($"BAND_BOTTOM_Y={usable[1]:R}"); Console.WriteLine($"BAND_LEFT_X={usable[0]:R}"); Console.WriteLine($"BAND_RIGHT_X={usable[2]:R}");
        positionAudit.Add((a,beforePosition,best.Position,best.Side,best.Position));
    ...
    dm.EditRebuild3(); dm.ForceRebuild3(false);
    foreach(var audit in positionAudit) {
        var afterPosition=AnnotationPosition(audit.Annotation.Display); var dx=afterPosition.Length>0?afterPosition[0]-audit.Assigned[0]:double.NaN; var dy=afterPosition.Length>1?afterPosition[1]-audit.Assigned[1]:double.NaN;
        var match=afterPosition.Length>=2 && Math.Abs(dx)<=1e-9 && Math.Abs(dy)<=1e-9;
        Console.WriteLine($"ANNOTATION_ID={audit.Annotation.Id}"); Console.WriteLine($"TEXT_POSITION_ASSIGNED_X={audit.Assigned[0]:R}"); Console.WriteLine($"TEXT_POSITION_ASSIGNED_Y={audit.Assigned[1]:R}");
        Console.WriteLine($"TEXT_POSITION_AFTER_REBUILD_X={(afterPosition.Length>0?afterPosition[0]:double.NaN):R}"); Console.WriteLine($"TEXT_POSITION_AFTER_REBUILD_Y={(afterPosition.Length>1?afterPosition[1]:double.NaN):R}");
        Console.WriteLine($"POSITION_DELTA_X={dx:R}"); Console.WriteLine($"POSITION_DELTA_Y={dy:R}"); Console.WriteLine($"POSITION_APPLICATION_MATCH={(match?"YES":"NO")}");
    }
    Console.WriteLine("ANNOTATION_LAYOUT_STAGE_EXIT=YES");
    ...
    // TEMPORARY PDF-FIRST DELIVERY: no SLDDRW persistence or reopen before export.
    var overallRequested = plan.GetProperty("intents").EnumerateArray().Count(x => x.GetProperty("kind").GetString() == "OVERALL");
    var overallCreated = annotations.Count(a => a.Kind == "OVERALL" && a.Display is not null);
    var overallSystemMatches = annotations.Count(a => a.Kind == "OVERALL" && a.Display is not null && ((Dimension)a.Display.GetDimension()).SystemValue > 0);
    var wrongDimensions = 0;
    Console.WriteLine("SLDDRW_PERSISTENCE_BEFORE_PDF=DISABLED");
    Console.WriteLine($"OVERALL_REQUESTED={overallRequested}");
    Console.WriteLine($"OVERALL_CREATED={overallCreated}");
    Console.WriteLine($"OVERALL_SYSTEMVALUE_MATCH={overallSystemMatches}");
    Console.WriteLine($"WRONG_DIMENSIONS={wrongDimensions}");
    if (overallRequested != 3 || overallCreated != 3 || overallSystemMatches != 3 || wrongDimensions != 0)
        throw new InvalidOperationException("PDF_EXPORT_BLOCKED_OVERALL_GATE");
    var pdf = Path.Combine(output, "DP90_current_delivery_layout_v7.pdf");
    Directory.CreateDirectory(Path.GetDirectoryName(pdf)!);
    var pdfActiveError = 0;
    var activeDrawingForPdf = sw.ActivateDoc3(dm.GetTitle(), false, (int)swRebuildOnActivation_e.swDontRebuildActiveDoc, ref pdfActiveError) as ModelDoc2;
    if (activeDrawingForPdf is null) throw new InvalidOperationException("DRAWING_ACTIVATION_FAILED_BEFORE_PDF");
    activeDrawingForPdf.ClearSelection2(true);
    int pdfErrors = 0, pdfWarnings = 0;
    Console.WriteLine("PDF_EXPORT_ENTER=YES");
    var pdfReturned = activeDrawingForPdf.Extension.SaveAs(pdf, (int)swSaveAsVersion_e.swSaveAsCurrentVersion, (int)swSaveAsOptions_e.swSaveAsOptions_Silent, null, ref pdfErrors, ref pdfWarnings);
    Console.WriteLine($"PDF_EXPORT_RETURNED={pdfReturned}");
    Console.WriteLine($"PDF_EXPORT_RESULT={pdfReturned}");
    Console.WriteLine($"PDF_EXPORT_ERRORS={pdfErrors}");
    Console.WriteLine($"PDF_EXPORT_WARNINGS={pdfWarnings}");
    var pdfExists = File.Exists(pdf);
    var pdfSize = pdfExists ? new FileInfo(pdf).Length : 0;
    Console.WriteLine($"PDF_EXISTS={pdfExists}");
    Console.WriteLine($"PDF_SIZE={pdfSize}");
    if (!pdfReturned || !pdfExists || pdfSize <= 0) throw new InvalidOperationException("PDF_EXPORT_FAILED");
    Console.WriteLine("DELIVERY_PDF_PRESERVED=YES");
    return 0;
}
...
static int RunOwnedPipeline(string[] argv)
{
    ...
    try
    {
        ...
        using var acceptedPlanDocument = JsonDocument.Parse(File.ReadAllText(planPath));
        var acceptedFacts = TryLoadSemanticExecutionFacts(acceptedPlanDocument.RootElement, semDoc.RootElement, semanticsPath);
        ...
        ExecuteAcceptedDrawingCore(new AcceptedDrawingExecutionContext(session, sw, model, acceptedFacts, outputDir, stem), planPath, templatePath);
...
static ResolvedAnnotationExecutionContext ResolveAnnotationExecutionContext(AnnotationExecutionRequest request, DrawingExecutionContext context, GeometryResolution? geometry = null)
{
    if (request is null) throw new ArgumentNullException(nameof(request));
    if (request.Definition is null || string.IsNullOrWhiteSpace(request.Definition.AnnotationId))
        return new ResolvedAnnotationExecutionContext(request, null, geometry, false, "UNRESOLVED_DEFINITION", false, "DEFINITION_OR_IDENTITY_MISSING");
    if (string.IsNullOrWhiteSpace(request.OwnerViewKey))
        return new ResolvedAnnotationExecutionContext(request, null, geometry, request.SemanticTargetM.HasValue, "UNRESOLVED_OWNER_VIEW_KEY", false, "OWNER_VIEW_KEY_MISSING");
    if (!context.RuntimeViews.TryGetValue(request.OwnerViewKey, out var ownerView) || ownerView is null)
        return new ResolvedAnnotationExecutionContext(request, null, geometry, request.SemanticTargetM.HasValue, "OWNER_VIEW_NOT_FOUND", false, "OWNER_VIEW_NOT_FOUND_IN_RUNTIME_VIEWS");
    return new ResolvedAnnotationExecutionContext(request, ownerView, geometry, request.SemanticTargetM.HasValue, "RESOLVED_FROM_RUNTIME_VIEWS", true, null);
}

static GeometryResolution? TryResolveAnnotationGeometry(ResolvedAnnotationExecutionContext context, DrawingExecutionContext executionContext)
{
    if (!context.IsResolved || context.OwnerView is null || context.Request.Definition is null || string.IsNullOrWhiteSpace(context.Request.Definition.AnnotationId))
    {
        context.IsResolved = false;
        context.FailureReason ??= "ANNOTATION_GEOMETRY_INPUT_UNRESOLVED";
        return null;
    }
    if (executionContext.MathUtility is null || executionContext.Facts.DatumEvidence is null || executionContext.Facts.FrameEvidence is null)
    {
        context.IsResolved = false;
        context.FailureReason = executionContext.MathUtility is null ? "MATH_UTILITY_UNAVAILABLE" : "SEMANTIC_GEOMETRY_EVIDENCE_UNAVAILABLE";
        return null;
    }
    try
    {
        var geometry = ResolveGeometry(
            executionContext.DrawingModel,
            executionContext.MathUtility,
            context.OwnerView,
            context.Request.OwnerViewRole,
            context.Request.Definition,
            executionContext.Facts.HoleSemantics,
            executionContext.Facts.PhysicalOpenings,
            executionContext.Facts.DatumEvidence,
            executionContext.Facts.FrameEvidence);
        context.Geometry = geometry;
        context.IsResolved = geometry is not null;
        context.FailureReason = geometry is null ? "ANNOTATION_GEOMETRY_UNAVAILABLE" : null;
        return geometry;
    }
...
static NativeAnnotationRouterResult ExecuteAnnotationCreation(ResolvedAnnotationExecutionContext context, DrawingExecutionContext executionContext)
{
    var kind = context.Request.AnnotationKind.Trim().ToUpperInvariant();
    if (!context.IsResolved || context.OwnerView is null || context.Request.Definition is null || string.IsNullOrWhiteSpace(context.Request.Definition.AnnotationId) || (kind != "OVERALL" && context.Geometry is null))
        return new NativeAnnotationRouterResult(false, null, "ANNOTATION_EXECUTION_CONTEXT_UNRESOLVED");
    if (kind == "OVERALL")
        return ExecuteOverallAnnotation(context, executionContext);

    try
    {
        object nativeResult = kind switch
        {
            "PATTERN" => CreatePitchDimension(executionContext.DrawingModel, context.OwnerView, context.Request.Definition, context.Geometry!, 0),
            "CALLOUT" => CreateHoleCallout(executionContext.DrawingModel, context.OwnerView, context.Request.Definition, context.Geometry!, 0),
            _ => throw new InvalidOperationException($"UNSUPPORTED_ANNOTATION_KIND:{context.Request.AnnotationKind}")
        };
        return new NativeAnnotationRouterResult(true, nativeResult, null);
    }
    catch (Exception error)
    {
        return new NativeAnnotationRouterResult(false, null, error.Message);
    }
}

static NativeAnnotationRouterResult ExecuteOverallAnnotation(ResolvedAnnotationExecutionContext context, DrawingExecutionContext executionContext)
{
    Console.WriteLine("OVERALL_EXECUTION_ENTER=YES");
    var overall = context.Request.OverallContext;
    var valid = context.IsResolved && context.OwnerView is not null && overall is not null && overall.SupportEvidence.Count > 0 && executionContext.MathUtility is not null;
    Console.WriteLine($"OVERALL_CONTEXT_VALID={(valid ? "YES" : "NO")}");
    Console.WriteLine($"OVERALL_SUPPORT_EVIDENCE_COUNT={(overall?.SupportEvidence.Count ?? 0)}");
    Console.WriteLine($"OVERALL_CONTEXT_BINDING={(overall is null ? "NO" : "YES")}");
    Console.WriteLine($"OVERALL_CONTEXT_SOURCE={(overall is null ? "UNAVAILABLE" : "INLINE_PREPARATION")}");
    Console.WriteLine($"OVERALL_SUPPORT_EVIDENCE_BOUND={(overall?.SupportEvidence.Count ?? 0)}");
    Console.WriteLine($"OVERALL_TARGET_ENVELOPE_BOUND={(overall is not null ? "YES" : "NO")}");
    Console.WriteLine($"OVERALL_ROUTER_INPUT_COMPLETE={(valid ? "YES" : "NO")}");
    if (!valid) { Console.WriteLine("OVERALL_EXECUTION_EXIT=YES"); return new NativeAnnotationRouterResult(false, null, "OVERALL_EXECUTION_CONTEXT_UNRESOLVED"); }
    var search = FindBoundedSupportReferenceCandidates(overall!.SupportEvidence, overall.AxisIndex, overall.TargetMinM, overall.TargetMaxM, overall.TargetAxisSpanM, overall.ScaleToleranceM, executionContext.MathUtility!, context.OwnerView!, overall.Orientation);
    Console.WriteLine($"OVERALL_CANDIDATE_COUNT={search.Candidates.Count}");
    OverallProbeResult? accepted = null;
    foreach (var candidate in search.Candidates)
    {
        var outcome = ProbeOverallCandidate(candidate, executionContext.DrawingModel, executionContext.MathUtility!, context.OwnerView!, context.Request.Definition.AnnotationId, context.Request.OwnerViewRole, overall.Axis, overall.AxisIndex, overall.Orientation, overall.SemanticTargetM, overall.ExpectedValidationTargetM, overall.SupportEvidence.Count, null);
        Console.WriteLine($"OVERALL_PROBE_OUTCOME={outcome.Kind}");
        if (outcome.Kind == OverallProbeOutcomeKind.Accepted) { accepted = outcome.Result; break; }
    }
    if (accepted is null) { Console.WriteLine("OVERALL_EXECUTION_EXIT=YES"); return new NativeAnnotationRouterResult(false, null, "OVERALL_NO_ACCEPTED_PROBE"); }
    Console.WriteLine("OVERALL_COMMIT_ENTER=YES");
    var committed = CommitOverallAnnotation(accepted, executionContext.DrawingModel, executionContext.MathUtility!, context.OwnerView!, context.Request.Definition.AnnotationId, context.Request.OwnerViewRole, overall.Axis, overall.Orientation, overall.SemanticTargetM, overall.SupportEvidence.Count, accepted.Strategy);
    Console.WriteLine("OVERALL_VALIDATION_RESULT=PASS");
    var endpoints = committed.Geometry.Entities.Select(e => ProjectModelPointToSheet(executionContext.MathUtility!, context.OwnerView!, e.ModelPoint)).ToArray();
    var text = (overall.SemanticTargetM * 1000.0).ToString("0.###", System.Globalization.CultureInfo.InvariantCulture);
    context.Request.RegistrationAccumulator.Add(new PolicyAnnotation(context.Request.Definition.AnnotationId, "OVERALL", context.Request.OwnerViewRole, committed.Display, endpoints, text, overall.Orientation, committed.NativeResult));
    Console.WriteLine("OVERALL_REGISTERED=YES");
    Console.WriteLine("OVERALL_NATIVE_ROUTED_REAL=YES");
    Console.WriteLine("OVERALL_EXECUTION_EXIT=YES");
    return new NativeAnnotationRouterResult(true, committed.NativeResult, null);
}
...
{
    Console.WriteLine("PDF_DELIVERY_FLOW_ENTER=YES");
    Console.WriteLine("SHARED_EXECUTOR_HAS_CANONICAL_SEMANTIC_FACTS=YES");
    Console.WriteLine($"SHARED_EXECUTOR_SEMANTIC_FACT_COUNT={context.Facts.PlanDefinitionCount}");
    ...
    var runtimeAnnotationRegistry = new List<RuntimeAnnotationHandle>();
    foreach (var view in views)
    {
        for (var display = view.GetFirstDisplayDimension5() as DisplayDimension; display is not null; display = display.GetNext5() as DisplayDimension)
        {
            try
            {
                var nativeAnnotation = display.GetAnnotation() as Annotation;
                if (nativeAnnotation is null) continue;
                var id = display.GetNameForSelection();
                runtimeAnnotationRegistry.Add(new RuntimeAnnotationHandle(id, "NATIVE_DISPLAY_DIMENSION", view.Name, nativeAnnotation, display, null));
                Console.WriteLine($"RUNTIME_ANNOTATION_REGISTER annotation_id={id};annotation_kind=NATIVE_DISPLAY_DIMENSION;owner_view_role={view.Name};native_type=DisplayDimension;registration_result=YES;reason=EXISTING_NATIVE_OBJECT");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"RUNTIME_ANNOTATION_REGISTER annotation_id=<unavailable>;annotation_kind=NATIVE_DISPLAY_DIMENSION;owner_view_role={view.Name};native_type=DisplayDimension;registration_result=NO;reason={ex.Message}");
            }
        }
    }
    Console.WriteLine($"RUNTIME_ANNOTATION_REGISTRY_COUNT={runtimeAnnotationRegistry.Count}");
    Console.WriteLine($"RUNTIME_OVERALL_HANDLE_COUNT={runtimeAnnotationRegistry.Count(x => x.AnnotationKind.Equals("OVERALL", StringComparison.OrdinalIgnoreCase))}");
    Console.WriteLine($"RUNTIME_PATTERN_HANDLE_COUNT={runtimeAnnotationRegistry.Count(x => x.AnnotationKind.Equals("PATTERN", StringComparison.OrdinalIgnoreCase))}");
    Console.WriteLine($"RUNTIME_CALLOUT_HANDLE_COUNT={runtimeAnnotationRegistry.Count(x => x.AnnotationKind.Equals("CALLOUT", StringComparison.OrdinalIgnoreCase))}");
    Console.WriteLine($"RUNTIME_ANNOTATION_COLLECTION_COUNT_AFTER_CREATION={runtimeAnnotationRegistry.Count}");
    Console.WriteLine("PDF_DELIVERY_ANNOTATIONS_COMPLETE=YES");
    Console.WriteLine("ANNOTATION_LAYOUT_STAGE_ENTER=YES");
    Console.WriteLine($"ANNOTATION_LAYOUT_INPUT_COUNT={runtimeAnnotationRegistry.Count}");
    Console.WriteLine($"ANNOTATION_LAYOUT_MOVABLE_COUNT={runtimeAnnotationRegistry.Count(x => x.NativeAnnotation is not null)}");
    foreach (var handle in runtimeAnnotationRegistry)
    {
        var position = AnnotationPosition(handle.NativeDisplayDimension);
        Console.WriteLine($"ANNOTATION_ID={handle.AnnotationId}");
        Console.WriteLine($"ANNOTATION_KIND={handle.AnnotationKind}");
        Console.WriteLine($"OWNER_VIEW_ROLE={handle.OwnerViewRole}");
        Console.WriteLine($"TEXT_POSITION_BEFORE_X={(position.Length > 0 ? position[0] : double.NaN):R}");
        Console.WriteLine($"TEXT_POSITION_BEFORE_Y={(position.Length > 1 ? position[1] : double.NaN):R}");
    }
    if (runtimeAnnotationRegistry.Count == 0) Console.WriteLine("ANNOTATION_LAYOUT_BLOCKER=EMPTY_RUNTIME_ANNOTATION_COLLECTION");
    else if (runtimeAnnotationRegistry.All(x => x.NativeAnnotation is null)) Console.WriteLine("ANNOTATION_LAYOUT_BLOCKER=NO_MOVABLE_NATIVE_ANNOTATIONS");
    Console.WriteLine("ANNOTATION_LAYOUT_STAGE_EXIT=YES");
    drawing.EditRebuild3(); drawing.ClearSelection2(true);
    ...
    Console.WriteLine("PDF_DELIVERY_BEFORE_EXPORT=YES");
...
static object CreateHoleCallout(ModelDoc2 model, View owner, Definition definition, GeometryResolution geometry, int lane)
{
    ...
    try
    {
        if (model is DrawingDoc drawingForActivation)
        {
            Console.WriteLine("CALLOUT_VIEW_ACTIVATE_BEFORE=YES");
            drawingForActivation.ActivateView(owner.Name);
            Console.WriteLine($"CALLOUT_ACTIVE_VIEW_AFTER={owner.Name}");
        }
        Console.WriteLine("CALLOUT_SELECTION_CLEAR_BEFORE=YES");
        model.ClearSelection2(true);
        Console.WriteLine("CALLOUT_RESELECT_ENTITY=YES");
        ...
        Console.WriteLine($"selected_count={selectedCount}");
        Console.WriteLine($"call_reselected_entity_type={entity.NativeType}");
        Console.WriteLine("call_reselected_selection_mark=0");
        ...
        var position = DimensionPosition(owner, definition, lane);
        Console.WriteLine("CALLOUT_REBUILD_BEFORE_NATIVE=YES");
        model.EditRebuild3();
        var nativeCallClock = Stopwatch.StartNew();
        Console.WriteLine("CALLOUT_NATIVE_CALL_ENTER=YES");
        Console.WriteLine($"call_owner_view_role={GetOwnerRole(definition)}");
        Console.WriteLine($"call_selected_entity_type={entity.NativeType}");
        Console.WriteLine($"call_selected_entity_count={selectedCount}");
        Console.WriteLine("call_selection_mark=0");
        Console.WriteLine($"call_active_view={owner.Name}");
        Console.WriteLine($"call_document_type={model.GetType()}");
        Console.WriteLine("call_rebuild_state=PREVIOUS_REBUILD_COMPLETE");
        object? created = drawing.AddHoleCallout2(position[0], position[1], 0.0);
        nativeCallClock.Stop();
        Console.WriteLine("CALLOUT_NATIVE_CALL_RETURN=YES");
        Console.WriteLine($"call_elapsed_ms={nativeCallClock.ElapsedMilliseconds}");
        var display = created as DisplayDimension;
        ...
        Console.WriteLine($"CALLOUT_NATIVE_CALL_TIMEOUT={(nativeCallClock.ElapsedMilliseconds > 30000 ? "YES" : "NO")}");
        model.ClearSelection2(true);
        if (isHoleCallout)
...
static OverallProbeOutcome ProbeOverallCandidate(SupportReferencePairCandidate candidate, ModelDoc2 model, MathUtility mathUtility, View owner, string annotationId, string ownerRole, string axis, int axisIndex, string orientation, double semanticTargetM, double expectedM, int rawCount, Stopwatch? overallCreateClock)
{
    Console.WriteLine($"OVERALL_SUPPORT_PAIR_CANDIDATE reference_type_A={candidate.A.NativeType};reference_type_B={candidate.B.NativeType};support_coordinate_A={candidate.A.ModelPoint[axisIndex]};support_coordinate_B={candidate.B.ModelPoint[axisIndex]};candidate_axis_span={Math.Abs(candidate.B.ModelPoint[axisIndex]-candidate.A.ModelPoint[axisIndex])};extent_error_m={Math.Abs(Math.Abs(candidate.B.ModelPoint[axisIndex]-candidate.A.ModelPoint[axisIndex])-semanticTargetM)};projected_dx={candidate.Pb[0]-candidate.Pa[0]};projected_dy={candidate.Pb[1]-candidate.Pa[1]};selectable_A=true;selectable_B=true");
    Console.WriteLine($"OVERALL_SUPPORT_CANDIDATE_ATTEMPT annotation_id={annotationId};reference_type_A={candidate.A.NativeType};reference_type_B={candidate.B.NativeType};candidate_axis_span={Math.Abs(candidate.B.ModelPoint[axisIndex]-candidate.A.ModelPoint[axisIndex])};semantic_target_m={semanticTargetM}");
    Console.WriteLine($"OVERALL_SELECTED_REFERENCE annotation_id={annotationId};reference_type_A={candidate.A.NativeType};reference_type_B={candidate.B.NativeType};duration_ms={overallCreateClock?.ElapsedMilliseconds ?? 0};temporary_candidate=true");
    var candidateGeometry = new GeometryResolution(new[] { candidate.A, candidate.B }, axis, null, null, expectedM * 1000, rawCount, null);
    var candidateDefinition = new Definition { AnnotationId = annotationId, Category = "OVERALL_DIMENSION", OwnerView = JsonSerializer.SerializeToElement(ownerRole), Geometry = JsonSerializer.SerializeToElement(new { endpoint_a = new { axis }, endpoint_b = new { axis } }), Dimension = JsonSerializer.SerializeToElement(new { orientation }), Placement = new Placement { Lane = "OVERALL_DIMENSION", Side = "ABOVE" }, Creation = JsonSerializer.SerializeToElement(new { native_only = true }), Validation = JsonSerializer.SerializeToElement(new { }) };
    DisplayDimension? temporaryDisplay = null;
    OverallProbeOutcome? probeOutcome = null;
    try
    {
        var createdCandidate = CreateLinearDimension(model, mathUtility, owner, candidateDefinition, candidateGeometry, 0);
        ...
        temporaryDisplay = candidateId is null ? null : FindDisplayDimension(owner, candidateId);
        var candidateSystem = temporaryDisplay is null ? 0 : ReadPositiveSystemValue(temporaryDisplay.GetDimension() as Dimension);
        var candidateMatch = candidateSystem > 0 && Math.Abs(candidateSystem - semanticTargetM) <= 0.0001;
        Console.WriteLine($"OVERALL_SUPPORT_CANDIDATE_RESULT annotation_id={annotationId};creation_result={(temporaryDisplay is null ? "FAILED" : "CREATED")};actual_system_value_m={candidateSystem:0.#########};absolute_error_m={Math.Abs(candidateSystem-semanticTargetM):0.#########};semantic_validation={(candidateMatch ? "PASS" : "FAIL")}");
        if (candidateMatch)
        {
            var result = new OverallProbeResult(candidate.A, candidate.B, candidate.Pa, candidate.Pb, candidate.Score, "BOUNDED_SEARCH", candidateSystem, semanticTargetM);
            probeOutcome = new OverallProbeOutcome(OverallProbeOutcomeKind.Accepted, result, null, null);
        }
        else
        {
            probeOutcome = new OverallProbeOutcome(OverallProbeOutcomeKind.NormalRejected, null, "SEMANTIC_SYSTEMVALUE_MISMATCH", null);
        }
    }
    catch (Exception candidateError)
    {
        Console.WriteLine($"OVERALL_CANDIDATE_REJECTED annotation_id={annotationId};reference_type_A={candidate.A.NativeType};reference_type_B={candidate.B.NativeType};reason={candidateError.Message};duration_ms={overallCreateClock?.ElapsedMilliseconds ?? 0}");
        Console.WriteLine($"OVERALL_SUPPORT_CANDIDATE_RESULT annotation_id={annotationId};creation_result=FAILED;semantic_validation=FAIL;rejection_reason={candidateError.Message}");
        probeOutcome = new OverallProbeOutcome(OverallProbeOutcomeKind.ExceptionContinue, null, "CANDIDATE_PROBE_EXCEPTION", candidateError.Message);
    }
    finally
    {
        if (temporaryDisplay is not null)
        {
            Console.WriteLine($"OVERALL_TEMP_DIM_DELETE annotation_id={annotationId};reason=PROBE_FINALLY_CLEANUP;duration_ms={overallCreateClock?.ElapsedMilliseconds ?? 0}");
            try
            {
                DeleteAnnotation(model, temporaryDisplay);
            }
            catch (Exception cleanupError)
            {
                Console.WriteLine($"OVERALL_TEMP_DIM_DELETE_FAILED annotation_id={annotationId};exception_type={cleanupError.GetType().Name};message={cleanupError.Message}");
                probeOutcome = new OverallProbeOutcome(OverallProbeOutcomeKind.ExceptionContinue, null, "PROBE_CLEANUP_EXCEPTION", cleanupError.Message);
            }
        }
    }
    return probeOutcome ?? new OverallProbeOutcome(OverallProbeOutcomeKind.ExceptionContinue, null, "CANDIDATE_PROBE_NO_OUTCOME", null);
}

static OverallCommitResult CommitOverallAnnotation(OverallProbeResult acceptedProbe, ModelDoc2 model, MathUtility mathUtility, View owner, string annotationId, string ownerRole, string axis, string orientation, double semanticTargetM, int rawCount, string strategy)
{
    var geometry = new GeometryResolution(new[] { acceptedProbe.A, acceptedProbe.B }, axis, null, null, semanticTargetM * 1000.0, rawCount, null);
    var definition = new Definition { AnnotationId = annotationId, Category = "OVERALL_DIMENSION", OwnerView = JsonSerializer.SerializeToElement(ownerRole), Geometry = JsonSerializer.SerializeToElement(new { endpoint_a = new { axis }, endpoint_b = new { axis }, entity_strategy = "PROJECTED_AXIS_EXTREMA" }), Dimension = JsonSerializer.SerializeToElement(new { orientation }), Placement = new Placement { Lane = "OVERALL_DIMENSION", Side = "ABOVE" }, Creation = JsonSerializer.SerializeToElement(new { native_only = true }), Validation = JsonSerializer.SerializeToElement(new { }) };
    var nativeResult = CreateLinearDimension(model, mathUtility, owner, definition, geometry, 0);
    var json = JsonSerializer.SerializeToElement(nativeResult);
    var creationIdentity = json.GetProperty("creation_identity").GetString() ?? throw new InvalidOperationException("CREATED_ID_MISSING");
    DisplayDimension? display = null;
    for (var dd = owner.GetFirstDisplayDimension5() as DisplayDimension; dd != null; dd = dd.GetNext5() as DisplayDimension)
        if (dd.GetNameForSelection() == creationIdentity) display = dd;
    if (display is null) throw new InvalidOperationException("CREATED_DISPLAY_MISSING");
    var nativeSystemValueM = ((Dimension)display.GetDimension()).SystemValue;
    const double acceptanceToleranceM = 0.0001;
    var absoluteErrorM = Math.Abs(nativeSystemValueM - semanticTargetM);
    var semanticValueMatch = absoluteErrorM <= acceptanceToleranceM;
    Console.WriteLine($"OVERALL_SYSTEM_VALUE_READ annotation_id={annotationId};system_value_m={nativeSystemValueM:0.#########};semantic_target_m={semanticTargetM:0.#########};absolute_error_m={absoluteErrorM:0.#########};semantic_value_match={semanticValueMatch}");
    Console.WriteLine($"OVERALL_SUPPORT_REFERENCE_RESULT annotation_id={annotationId};strategy={strategy};binding_result={(semanticValueMatch ? "SUCCESS" : "REJECTED")};creation_result={(semanticValueMatch ? "SUCCESS" : "REJECTED")};actual_system_value_m={nativeSystemValueM:0.#########};semantic_validation={(semanticValueMatch ? "PASS" : "FAIL")};rejection_reason={(semanticValueMatch ? "<none>" : "SEMANTIC_SYSTEMVALUE_MISMATCH")}");
    var relativeError = semanticTargetM == 0.0 ? (absoluteErrorM == 0.0 ? 0.0 : double.PositiveInfinity) : absoluteErrorM / Math.Abs(semanticTargetM);
    Console.WriteLine($"OVERALL_SEMANTIC_VALIDATION annotation_id={annotationId};semantic_target_m={semanticTargetM:0.#########};actual_system_value_m={nativeSystemValueM:0.#########};absolute_error_m={absoluteErrorM:0.#########};relative_error={relativeError:0.#########};acceptance_tolerance_m={acceptanceToleranceM:0.#########};semantic_value_match={semanticValueMatch}");
    if (!semanticValueMatch)
    {
        DeleteAnnotation(model, display);
        throw new InvalidOperationException($"SEMANTIC_SYSTEMVALUE_MISMATCH: {annotationId}; expected={semanticTargetM}m; actual={nativeSystemValueM}m");
    }
    return new OverallCommitResult(geometry, definition, nativeResult, display, nativeSystemValueM, creationIdentity);
}
...
static SemanticFrameEvidence LoadSemanticFrameEvidence(string path)
{
    ...
    if (extrema.Count == 0 && document.RootElement.TryGetProperty("bounding_box", out var boundingBox) && boundingBox.ValueKind == JsonValueKind.Object && boundingBox.TryGetProperty("exact_envelope", out var exactEnvelope) && exactEnvelope.ValueKind == JsonValueKind.Object && exactEnvelope.TryGetProperty("axis_evidence", out var axisEvidence) && axisEvidence.ValueKind == JsonValueKind.Object)
    {
        foreach (var axisProperty in axisEvidence.EnumerateObject())
        {
            var evidence = axisProperty.Value;
            var axis = TryString(evidence, "axis") ?? axisProperty.Name;
            var low = TryNumber(evidence, "low_coordinate", "low_coordinate_m");
            var high = TryNumber(evidence, "high_coordinate", "high_coordinate_m");
            var lowProven = evidence.TryGetProperty("low_proven", out var lowProvenElement) && lowProvenElement.ValueKind == JsonValueKind.True;
            var highProven = evidence.TryGetProperty("high_proven", out var highProvenElement) && highProvenElement.ValueKind == JsonValueKind.True;
            if (!string.IsNullOrWhiteSpace(axis) && lowProven && highProven && low is double lowCoordinate && high is double highCoordinate)
            {
                extrema.Add(new SemanticAxisExtreme(axis.Trim().ToUpperInvariant(), lowCoordinate, -1));
                extrema.Add(new SemanticAxisExtreme(axis.Trim().ToUpperInvariant(), highCoordinate, 1));
            }
        }
    }
    var axes = extrema
...
static SemanticExecutionFacts TryLoadSemanticExecutionFacts(JsonElement plan, JsonElement semantics, string semanticPath)
{
    var planDefinitionCount = plan.TryGetProperty("intents", out var intents) && intents.ValueKind == JsonValueKind.Array ? intents.GetArrayLength() : 0;
    if (planDefinitionCount == 0) throw new InvalidOperationException("SEMANTIC_FACTS_PLAN_DEFINITIONS_MISSING");
    var holes = LoadHoleSemantics(semanticPath);
    var physical = LoadPhysicalOpeningFacts(semantics);
    var frame = LoadSemanticFrameEvidence(semanticPath);
    DatumSemanticCatalog? datum = null;
    if (semantics.TryGetProperty("datums", out _)) datum = LoadDatumSemantics(semanticPath);
    return new SemanticExecutionFacts(planDefinitionCount, semantics, holes, physical, frame, datum);
}
static ResolvedAnnotationIntent? TryResolveAnnotationIntent(Definition definition, string ownerViewRole, GeometryResolution? geometry, bool semanticTargetResolved, out string reason)
{
    Console.WriteLine($"RESOLVED_INTENT_START annotation_id={definition.AnnotationId};kind={definition.Category}");
    if (geometry is null) { reason = "GEOMETRY_RESOLUTION_UNAVAILABLE"; return null; }
    if (geometry.Entities.Count == 0) { reason = "GEOMETRY_RESOLUTION_EMPTY"; return null; }
    if (string.IsNullOrWhiteSpace(ownerViewRole)) { reason = "OWNER_VIEW_ROLE_UNAVAILABLE"; return null; }
    if (!semanticTargetResolved && definition.Category is "OVERALL_DIMENSION" or "STEP_DIMENSION" or "HOLE_PITCH") { reason = "SEMANTIC_TARGET_UNAVAILABLE"; return null; }
    reason = "<none>";
    Console.WriteLine($"RESOLVED_INTENT_RESULT annotation_id={definition.AnnotationId};kind={definition.Category};owner_view_role={ownerViewRole};geometry_resolution_status=RESOLVED;semantic_target_status={(semanticTargetResolved ? "RESOLVED" : "NOT_REQUIRED")};resolution_status=RESOLVED;reason=<none>");
    return new ResolvedAnnotationIntent(definition, definition.AnnotationId, definition.Category, ownerViewRole, geometry, semanticTargetResolved);
}
sealed record ResolvedAnnotationIntent(Definition Definition, string AnnotationId, string Kind, string OwnerViewRole, GeometryResolution Geometry, bool SemanticTargetResolved);
sealed record SemanticExecutionFacts(int PlanDefinitionCount, JsonElement Semantics, IReadOnlyDictionary<string,HoleSemantic> HoleSemantics, IReadOnlyList<PhysicalOpeningFact> PhysicalOpenings, SemanticFrameEvidence? FrameEvidence, DatumSemanticCatalog? DatumEvidence);
sealed record OverallProbeResult(ResolvedNativeEntity A, ResolvedNativeEntity B, double[] ProjectedA, double[] ProjectedB, double Score, string Strategy, double ProbeSystemValueM, double SemanticTargetM);
sealed record OverallCommitResult(GeometryResolution Geometry, Definition Definition, object NativeResult, DisplayDimension Display, double NativeSystemValueM, string CreationIdentity);
sealed record AnnotationExecutionRequest(
    Definition Definition,
    string AnnotationKind,
    string OwnerViewRole,
    string OwnerViewKey,
    string Axis,
    string Orientation,
    double? SemanticTargetM,
    double? ExpectedValidationTargetM,
    double ValidationToleranceM,
    List<PolicyAnnotation> RegistrationAccumulator,
    OverallExecutionContext? OverallContext = null);
sealed class OverallExecutionContext
{
    public ResolvedAnnotationExecutionContext BaseContext { get; }
    public string Axis { get; }
    public string Orientation { get; }
    public double SemanticTargetM { get; }
    public double ExpectedValidationTargetM { get; }
    public IReadOnlyList<OverallSupportEvidence> SupportEvidence { get; }
    public int AxisIndex { get; }
    public double TargetMinM { get; }
    public double TargetMaxM { get; }
    public double TargetAxisSpanM { get; }
    public double ScaleToleranceM { get; }
    public OverallExecutionContext(ResolvedAnnotationExecutionContext baseContext, string axis, string orientation, double semanticTargetM, double expectedValidationTargetM, IReadOnlyList<OverallSupportEvidence> supportEvidence, int axisIndex, double targetMinM, double targetMaxM, double targetAxisSpanM, double scaleToleranceM)
    {
        BaseContext = baseContext;
        Axis = axis;
        Orientation = orientation;
        SemanticTargetM = semanticTargetM;
        ExpectedValidationTargetM = expectedValidationTargetM;
        SupportEvidence = supportEvidence;
        AxisIndex = axisIndex;
        TargetMinM = targetMinM;
        TargetMaxM = targetMaxM;
        TargetAxisSpanM = targetAxisSpanM;
        ScaleToleranceM = scaleToleranceM;
    }
}
sealed record NativeAnnotationRouterResult(bool Created, object? NativeResult, string? FailureReason);
sealed class ResolvedAnnotationExecutionContext
{
    public AnnotationExecutionRequest Request { get; }
    public View? OwnerView { get; }
    public GeometryResolution? Geometry { get; set; }
    public bool SemanticTargetResolved { get; }
    public string OwnerBindingState { get; set; }
    public bool IsResolved { get; set; }
    public string? FailureReason { get; set; }
    public ResolvedAnnotationExecutionContext(AnnotationExecutionRequest request, View? ownerView, GeometryResolution? geometry, bool semanticTargetResolved, string ownerBindingState, bool isResolved, string? failureReason)
    { Request = request; OwnerView = ownerView; Geometry = geometry; SemanticTargetResolved = semanticTargetResolved; OwnerBindingState = ownerBindingState; IsResolved = isResolved; FailureReason = failureReason; }
}
enum OverallProbeOutcomeKind { Accepted, NormalRejected, ExceptionContinue }
sealed record OverallProbeOutcome(OverallProbeOutcomeKind Kind, OverallProbeResult? Result, string? FailureReason, string? ExceptionMessage);
sealed class DrawingExecutionContext
{
    public SemanticExecutionFacts Facts { get; }
    public ModelDoc2 SourceModel { get; }
    public ModelDoc2 DrawingModel { get; }
    public DrawingDoc Drawing { get; }
    public IReadOnlyDictionary<string, View> RuntimeViews { get; }
    public MathUtility? MathUtility { get; }
    public DrawingExecutionContext(SemanticExecutionFacts facts, ModelDoc2 sourceModel, ModelDoc2 drawingModel, DrawingDoc drawing, IReadOnlyDictionary<string, View> runtimeViews, MathUtility? mathUtility = null)
    { Facts = facts; SourceModel = sourceModel; DrawingModel = drawingModel; Drawing = drawing; RuntimeViews = runtimeViews; MathUtility = mathUtility; }
}
...
sealed record RuntimeAnnotationHandle(string AnnotationId, string AnnotationKind, string OwnerViewRole, Annotation NativeAnnotation, DisplayDimension NativeDisplayDimension, Note? NativeNote);
