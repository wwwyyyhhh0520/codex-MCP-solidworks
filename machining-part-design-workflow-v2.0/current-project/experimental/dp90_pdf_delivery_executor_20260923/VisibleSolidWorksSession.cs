...
class AcceptedDrawingExecutionContext
{
    ...
    public AcceptedDrawingExecutionContext(VisibleSolidWorksSession session, SldWorks root, ModelDoc2 sourceModel, SemanticExecutionFacts facts, string outputDirectory, string outputStem)
    { Session = session; Root = root; SourceModel = sourceModel; Facts = facts; OutputDirectory = outputDirectory; OutputStem = outputStem; }
}
...
{
    public OwnedSolidWorksContext(VisibleSolidWorksSession session, SldWorks root, ModelDoc2 sourceModel, SemanticExecutionFacts facts, string outputDirectory, string outputStem)
        : base(session, root, sourceModel, facts, outputDirectory, outputStem) { }
}
