...
class DimensionPlan:
    ...
    required: bool
    status: str = "PLANNED"
    owner_view_role_requested: str | None = None
    owner_view_role_actual: str | None = None
    owner_resolution_source: str = "UNRESOLVED"
    owner_evidence: list[str] = field(default_factory=list)
    source_semantic_feature_id: str | None = None
    source_dimension_definition_id: str | None = None
...
def _selected_view_id(selected_view: Any, view_specs: list[dict], view_results: list[dict]) -> str | None:
    text = str(selected_view or "")
    for index, item in enumerate(view_results or [], start=1):
        if text and text == str(item.get("view_name") or ""):
            return f"V{index:03d}"
        if text and text == str(item.get("used") or ""):
            return f"V{index:03d}"
    # V10R's native overall reports use the stable display label
    # 工程图视图N for the actual view object.
    marker = "工程图视图"
    if marker in text:
        ...
        try:
            ...
            index = 0
        if 1 <= index <= len(view_specs or view_results or []):
            return f"V{index:03d}"
    return None


def _actual_owner(role_requested: Any, role_actual: Any, selected_view: Any, ids: dict[str, str], view_specs: list[dict], view_results: list[dict]) -> tuple[str | None, str | None, str, list[str]]:
    requested = _role(role_requested) or None
    # A V10R record may echo the requested role in ``view_role`` even when
    # no corresponding drawing view was created.  Treat it as actual only
    # when the role is present in the planner's real view map.
    candidate_actual = _role(role_actual) or None
    actual = candidate_actual if candidate_actual and ids.get(candidate_actual) else None
    evidence: list[str] = []
    owner_id = _selected_view_id(selected_view, view_specs, view_results)
    if owner_id:
        evidence.append("planner_execution.selected_view")
        if not actual:
            actual = next((role for role, value in ids.items() if value == owner_id), None)
    if owner_id is None and actual and ids.get(actual):
        owner_id = ids[actual]
        evidence.append("planner_execution.actual_view_role")
    if owner_id is None and requested and ids.get(requested):
        # This is only the existing planner-selected owner when no separate
        # execution record exists; it does not substitute a missing role.
        owner_id = ids[requested]
        actual = actual or requested
        evidence.append("planner_selected_view_role")
    if owner_id:
        source = "PLANNER_SELECTED_VIEW"
    else:
        source = "NO_EXECUTION_OWNER"
        evidence.append("no_actual_view_id_selected_by_planner_or_executor")
    return owner_id, actual, source, evidence
...
def build_drawing_plan(
    ...
    dimensions: list[DimensionPlan] = []
    for definition in dimension_definitions or []:
        definition_id = str(definition.get("id") or f"D{len(dimensions)+1:03d}")
        actual_record = next((item for item in semantic_dimensions.get("created") or [] if str(item.get("id") or item.get("definition_id") or "") == definition_id), {})
        owner_id, actual_role, owner_source, owner_evidence = _actual_owner(
            definition.get("preferred_view_role"), actual_record.get("view_role"), actual_record.get("selected_view"),
            ids, view_specs, view_results,
        )
        dimensions.append(DimensionPlan(
            ...
            owner_view_role_requested=_role(definition.get("preferred_view_role")) or None,
            owner_view_role_actual=actual_role,
            owner_resolution_source=owner_source,
            owner_evidence=owner_evidence,
            source_dimension_definition_id=definition_id,
        ))
    for index, candidate in enumerate(feature_dimension_candidates or [], start=1):
        dimension_id = f"FD{index:03d}"
        actual_record = next((item for item in feature_dimensions.get("created") or [] if item is candidate), {})
        if not actual_record:
            actual_record = next((item for item in feature_dimensions.get("created") or [] if item.get("feature_name") == candidate.get("feature_name") and item.get("dimension_role") == candidate.get("dimension_role") and item.get("distance_mm") == candidate.get("distance_mm")), {})
        requested_role = candidate.get("preferred_view_role")
        owner_id, actual_role, owner_source, owner_evidence = _actual_owner(requested_role, actual_record.get("view_role"), actual_record.get("selected_view"), ids, view_specs, view_results)
        dimensions.append(DimensionPlan(
            ...
            owner_view_role_requested=_role(requested_role) or None,
            owner_view_role_actual=actual_role,
            owner_resolution_source=owner_source,
            owner_evidence=owner_evidence,
            source_semantic_feature_id=str(candidate.get("feature_name") or "") or None,
        ))

    annotations: list[AnnotationPlan] = []
    for index, record in enumerate(native_hole_binding.get("records") or [], start=1):
        requested_role = record.get("requested_view_role") or record.get("preferred_view_role") or record.get("view_role")
        owner_id, actual_role, owner_source, owner_evidence = _actual_owner(requested_role, record.get("view_role"), record.get("selected_view"), ids, view_specs, view_results)
        annotations.append(AnnotationPlan(
            ...
            owner_resolution_source=owner_source,
            owner_evidence=owner_evidence,
            source_semantic_feature_id=str(record.get("feature_name") or "") or None,
        ))
